...
func (vi volumeInterfaceDboxed) WorkDirBase(vol types.BoxVolumeSpec) string {
...
func (vi volumeInterfaceDboxed) IsReadOnly(vol types.BoxVolumeSpec) bool {
	return false
}

func (vi volumeInterfaceDboxed) Create(ctx context.Context, vol types.BoxVolumeSpec) error {
...
func (vi volumeInterfaceDboxed) Delete(ctx context.Context, vol types.BoxVolumeSpec) error {
	workDir := getVolumeWorkDir(vi, vol)
...
func (vi volumeInterfaceDboxed) createS6Service(ctx context.Context, vol types.BoxVolumeSpec) error {
	name := vi.WorkDirBase(vol)
	workDir := getVolumeWorkDir(vi, vol)
	logDir := filepath.Join(types.DboxedDataDir, "logs/s6", name)
...
func (vi volumeInterfaceDboxed) deleteS6Service(ctx context.Context, vol types.BoxVolumeSpec) error {
	name := vi.WorkDirBase(vol)
	serviceDir := filepath.Join("/etc/services.d", name)
	symlinkPath := filepath.Join("/run/service", name)
...
func (vi volumeInterfaceDboxed) CheckRecreateNeeded(oldVol types.BoxVolumeSpec, newVol types.BoxVolumeSpec) bool {
