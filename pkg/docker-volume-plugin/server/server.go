...
func NewPluginServer(ctx context.Context, config plugin_config.Config) (*PluginServer, error) {
	...
	// s.volumes = volumes.New(plugin_config)
	// s.boxes = boxes.New(plugin_config)
	// s.machines = machines.New(plugin_config)
	// s.loadBalancers = load_balancers.New(plugin_config)
